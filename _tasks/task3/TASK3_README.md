...
####  Контекст
- После внедрения функции пользовательской отчетности значительно возросла нагрузка на аналитическую базу данных (ClickHouse). Пользователи часто запрашивают свои отчеты (за определенный период), при этом сами данные обновляются редко — только в момент срабатывания фонового ETL-процесса (DAG в Airflow). 
...
- Реализована новая версия API (`v2`), которая вместо генерации и отдачи "тяжелых" данных возвращает клиенту безопасную ссылку на статический файл в CDN.
...
- **Cache Miss (Первичный запрос):**
  1. Бэкенд запрашивает у БД "легкую" агрегацию `MAX(updated_at)` для заданного пользователя и дат.
  2. Бэкенд генерирует имя файла (хеш). Делает быстрый `HEAD`-запрос в MinIO. Файла нет.
  3. Бэкенд делает тяжелый `SELECT` в ClickHouse, собирает полный отчет.
  4. Бэкенд загружает JSON-файл в MinIO (S3) и возвращает фронтенду ссылку (например, `http://localhost:8095/cdn/hash.json`).
  5. Фронтенд делает GET-запрос по ссылке. Запрос попадает в Nginx.
  6. Nginx не находит файл в кэше, проксирует запрос в MinIO, скачивает файл, сохраняет его в свою директорию `/var/cache/nginx` и отдает фронтенду с заголовком `X-Cache-Status: MISS`.
- **Cache Hit (Вторичный запрос):**
  1. Бэкенд видит, что файл в MinIO уже есть, и мгновенно возвращает ту же самую ссылку.
  2. Фронтенд запрашивает файл у Nginx.
  3. Nginx находит файл в своем локальном кэше и отдает его мгновенно с заголовком `X-Cache-Status: HIT`, вообще не обращаясь к S3 и базе данных.
...
- **Решение:** 
  - Имя файла генерируется как префикс `batch` (в 4 задании появится префикс `stream`) + SHA-256 хеш от комбинации: `user_id` + `start_date` + `end_date` + `MAX(updated_at)` + `REPORT_LINK_SALT`.
...
- В `docker-compose3.yml` добавлены контейнеры `minio`, `nginx-cdn` и `minio-setup` (утилита для автоматического создания публичного бакета `reports` при старте стенда).
...
- **Что делает скрипт minio-setup** в контейнере?
  ...
  - Это избавит нас от необходимости каждый раз заходить в админку MinIO и настраивать права руками после пересоздания контейнеров.


#### 1.2 Последствия (обоснование)
- **Плюсы**:
  * Драматическое снижение нагрузки на ClickHouse: база выполняет тяжелые агрегации только один раз после обновления ETL.
...
5. Минимум дважды вытащить отчеты за период 20-22 февраля (данные по умолчанию инициализированы за 20-22 февраля) по кнопке `Get Report v2 (CDN OLTP batch)` ;
    - увидеть запросы с фронта на `nginx-cdn` (`localhost:8095/cdn/`) по ссылке ответа ручки `v2/reports`;
