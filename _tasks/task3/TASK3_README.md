...
>
>После внедрения фичи о получении пользовательской отчётности нагрузка на базу данных отчётности существенно возросла. Пользователи стали часто запрашивать свои отчёты. Но поскольку данные обновляются с помощью ETL-процесса по расписанию, на эти запросы пользователи получают одинаковые отчёты.
>
...
>2. При запросе отчёта сервис сначала проверяет наличие отчёта в S3. Если он там есть, то отдаёт ссылку на CDN. Если же отчёт не обнаружен, сервис должен его сгенерировать, положить в S3 и отдать ссылку на CDN в ответе. Для эмуляции CDN необходимо поднять и настроить Nginx как reverse proxy с включённым кешированием статических файлов.
...
- **Решение:** 
  - имя файла генерируется как `{userId}/{startDate}_{endDate}/{type}_{paramsHash}.json`, где:
    - часть {userId}/{startDate}_{endDate}/ позволяет, продолжая хранить имя в плоской структуре, получить такие преимущества:
      - S3 будет отображать файлы в структуре дерева; 
      - файлы можно будет проще найти вручную (хотя, конечно, это противоречивый плюс) и администрировать, например настроить правила перекачивания отдельных файлов в более дешевое хранилище;
  - `{type}` `batch_` (в 4 задании появится префикс `stream_`) позволяет разделить файлы от разных витрин;
  - `{paramsHash}` ответчает за криптографическую стойкость ссылки на файл, это SHA-256 хеш от комбинации: `user_id` + `start_date` + `end_date` + `MAX(updated_at)` + `REPORT_LINK_SALT`.
