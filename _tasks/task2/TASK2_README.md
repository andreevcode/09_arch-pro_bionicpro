...
- Текущая архитектура BionicPro использует две независимые базы данных PostgreSQL в качестве источников (OLTP);
  ...
  - **Telemetry**: хранит сырые логи активности протезов, поток в сотни тысяч записей в сутки с возможностью роста;
...
4. **Новый сервис `bionicpro-auth`** в составе API BionicPro:
   - который будет запрашивать и отдавать сгруппированные отчеты из витрины данных в ручке с параметрами:
      - пользователя `user_id` (**только для удобства тестирования на стенде, на проде пользователь должен вытаскиваться из JWT)**;
      - периоду (`start_date, end_date`); 
   - запросы он будет получать который из `bionicpro-auth`.
...
**Контекст:**
Требуется реализовать задачи 2–5 и внести правки в стенд в докере после 1 задания: автоматизировать сбор данных, создать витрину, ограничить доступ на уровне API и обновить UI.
...
5. Разделение DAG на два процесса:
   ...
   - **transformation (mart calculation)**: Daily, джойнить все таблицы staging слоя в витрину `daily_user_reports`. 
...
7. Новый сервис [bionicpro-reports](../../bionicpro-reports) на Spring Boot и Spring Security на Java.
   ...
   - ручка `/api/v1/reports` 
     ...
     - принимает параметр `user_id`:
       - чтобы удобно было тестировать на стенде;
       - поскольку пользователь не должен уметь читать чужие отчеты, параметр **следует убрать на проде** и вытаскивать пользователя из JWT;
     ...
     - умеет возвращать разные коды ответов: 
       ...
       - `401` или `403` для разных проблем с авторизацией:
         - протух JWT; 
         - `sub` из JWT не совпадает с `user_id`;
         - `user_id` в запросе (**для удоства тестирования ошибок авторизации на стенде, убрать на проде**) не совпадает с юзером в токене;
...
10. Нюансы:
    ...
    - требуется контроль за форматом дат при переносе из OLTP в OLAP, чтобы не потерять точность или часовые пояса;
...
5. **ВАЖНО**: на скриншотах видна **кнопка** скачивания отчета **"Get report"** для фронта в 2 версии задания, **но** в репозитории версия фронта для 4 задания, поэтому в этом задании тут и ниже **нужно использовать** кнопку **"Get Report v1 (Direct OLTP Batch)"**:
    - <img src="../task4/images/12_after_cdc_update.png" width="1150"/>    
6. Авторизоваться под одним из `ldap` пользователей из провести тесты (кроме `sarah.connor`):
   ...
   - **использовать другой `user_id` для такого теста**: в консоли браузера запрос с другим `user_id` должен возвращать 403:
     ```js
        ...
        ```
   - запросить данные за будущий период (например, 2026-02-24 - 2026-02-24) и получить 400 с текстом ошибки; то есть получить ответ, что отчет еще не подготовлен, а не пустой:
        -  <img src="images_ui/03_400_bad_request_check_report_availabilty.png" width="950"/>
        -  <img src="images_ui/04_400_bad_request_check_report_availabilty.png" width="950"/>
   - при этом запросы за прошлые периоды (до последнего дня, когда были отчеты) с пустым отчетом должны возвращаться с 200;
   - дождаться протухания AT, запросить отчет и получить 401;
   -  <img src="images_ui/01_browser_console_tests.png" width="1450"/>
7. Авторизоваться под `sarah.connor` и убедиться, что ее роль не позволяет запросить отчет по протезам (кнопка `Get Report v1 (Direct OLTP batch)`); все потому, что `sarah.connor` не доверяет роботизированным протезам =).    
