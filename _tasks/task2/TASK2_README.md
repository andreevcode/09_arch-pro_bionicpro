...
7. Новый сервис [bionicpro-reports](../../bionicpro-reports) на Spring Boot и Spring Security на Java.
   ...
   - новая ручка ручка `/api/v1/reports` 
...
4. Можно убедиться, что данные из oltp хранилища корректино загрузились в `clickhouse`, а затем правильно рассчиталась витрина.
...
5. Авторизоваться под одним из `ldap` пользователей из провести тесты (кроме `sarah.connor`): 
   - проверить доступ к своим отчетам за разные периоды (есть данные с 20 по 22 февраля) и убедиться, что все работает корректно;
...
6. Авторизоваться под `sarah.connor` и убедиться, что ее роль не позволяет запросить отчет по протезам (все потому, что она не доверяет роботизированным протезам).    
