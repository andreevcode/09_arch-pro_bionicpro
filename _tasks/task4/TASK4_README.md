...
---
### 4.1. ADR 10: стратегия внедрения CDC для формирования Near-Real-Time (NRT) аналитики
...
3. **API** Для сохранения обратной совместимости в ручку `/api/v2/reports` добавлен опциональный параметр `stream=false`. Контроллер и репозиторий динамически выбирают, из какой витрины (старой батчевой или новой потоковой) читать данные. 
...
2. **Кто должен триггерить витрину?**
   ...
   - Когда летит событие телеметрии, ClickHouse подхватывает его, идет в `staging` устройств, обогащает `user_id` и пишет результат в витрину.
...
6. Авторизоваться под одним из `ldap` пользователей из провести тесты (кроме `sarah.connor`) с новой кнопкой `Get Report v2 (CDN OLTP stream)`: 
