...
2. **Кто должен триггерить витрину?**
   ...
   - логика SQL построения витрины при возникновении нового микробатча событий телеметрии:
     - ClickHouse подхватывает микробатч, обогащает его в `staging` устройств и пользователей по `user_id`,`device_id` и пишет результат в витрину.
