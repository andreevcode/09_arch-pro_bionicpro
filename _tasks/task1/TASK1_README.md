...
8. **ВАЖНО**: на скриншотах видна **кнопка** скачивания отчета **"Download report"** для фронта в 1 версии задания, **но** в репозитории версия фронта для 4 задания, поэтому в этом задании тут и ниже **можно использовать любую кнопку**, например "Get Report v1 (Direct OLTP Batch)":
   - <img src="../task4/images/12_after_cdc_update.png" width="1150"/>
9. Затем для тестирования обновления `JSESSIONID`, а также времени жизни и ротации токенов стоит сделать несколько запросов на бизнес-бэкенд и дождаться, пока протухнет `RT` через 5 минут с момента авторизации:
...
10. **Примечание**: на скриншотах выше нигде не видно `CSRF` токена, его добавил чуть позже, сейчас он есть:
